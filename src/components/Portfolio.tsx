...
import { Briefcase, MapPin, Calendar, ArrowUpRight, X } from 'lucide-react';
    ...
    <section id="portfolio" className="py-20 lg:py-28 bg-[#11141a] relative overflow-hidden border-t border-b border-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-[11px] font-semibold font-['Space_Grotesk'] tracking-wider uppercase transition-all duration-300 ${
                      activeCategory === cat
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-[11px] font-semibold font-['Space_Grotesk'] tracking-wider uppercase transition-all duration-300 ${
                      activeSectorFilter === sec
                        ? 'bg-amber-500/20 border border-amber-500 text-amber-400 font-bold shadow-sm'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, idx) => (
            <AnimatedSection key={project.id} direction="up" delay={(idx % 3) * 100}>
              <div
                onClick={() => setSelectedProject(project)}
                className="group cursor-pointer rounded-2xl bg-slate-900/80 border border-slate-800 overflow-hidden glass-card-hover shadow-xl flex flex-col h-full"
              >
                {/* Image Container */}
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 overflow-y-auto animate-fade-in">
          <div className="bg-[#11141a] border border-amber-500/30 rounded-3xl max-w-3xl w-full overflow-hidden shadow-2xl my-8 relative transform transition-transform duration-300 scale-100">
            ...
            <div className="relative h-72 sm:h-96">
              ...
              <button
                ...
                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-slate-950/80 text-slate-200 hover:text-white flex items-center justify-center border border-slate-700 hover:scale-110 transition-transform"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="pt-4 border-t border-slate-800 flex justify-end gap-3">
                <button
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-bold font-['Space_Grotesk'] text-xs tracking-wider uppercase rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-500/20"
                >
